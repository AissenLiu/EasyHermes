...
- 离线启动：运行包内置 Python、Python wheels 和 WebUI 静态资源，内网 Windows 解压即用。
...
- 轻量内网包：只打包 WebUI 与 Hermes Agent 核心运行所需依赖，保留命令行、计划任务和 Windows PTY 支持。
...
2. 从 `packages/wheelhouse/` 离线安装 WebUI 和 Agent 所需 Python 依赖。
3. 创建本地 `data/`、`data/.hermes/`、`data/webui/` 和 `workspace/`。
4. 设置 `HERMES_HOME`、WebUI 状态目录、默认工作区和 Agent 源码路径。
5. 启动 WebUI 并打开 `http://127.0.0.1:8787`。
...
- 便携包包含 Windows 可用的 Hermes Agent 核心依赖、`cli`、`cron`、`pty` extras、WebUI 依赖，以及 WebUI 的本地静态资源。
