...
export async function bootstrap() {
  ...
  server.on('listening', () => {
    ...
    console.log(`Server: http://${config.host}:${config.port} (LAN: http://${localIp}:${config.port})`)
    console.log(`Upstream: ${config.upstream}`)
    console.log(`Log: ~/.hermes-web-ui/logs/server.log`)
    logger.info('Server: http://%s:%d (LAN: http://%s:%d)', config.host, config.port, localIp, config.port)
