...
目标很直接：在有网络的地方完成依赖下载和打包，然后把一个 zip 拷贝到内网 Windows 电脑，解压后双击 `start.bat` 即可运行，不要求内网机器安装 Docker、WSL、Node.js 或 Python。
...
- 离线启动：运行包内置 Python 运行时和常用依赖，内网 Windows 解压即用。
...
构建完成后，在该 workflow run 的 **Artifacts** 区域下载：

```text
EazyHermes-windows-offline-<tag>
```
...
- GitHub Actions artifact 默认保留 14 天，过期后需要重新手动运行 workflow。
- 如果修改了依赖清单，请重新打 tag 并用该 tag 手动打包。
- 便携包包含的是常用运行依赖；某些重型或平台相关能力可能仍需要额外环境，例如浏览器自动化、语音、特定本地工具链等。
