...
目标很直接：在有网络的地方完成必要依赖下载和打包，然后把一个 zip 从 GitHub Releases 下载并拷贝到内网 Windows 电脑，解压后双击 `start.bat` 即可运行，不要求内网机器安装 Docker、WSL、Node.js 或 Python。

## 项目组成

- `hermes-agent`：核心 Agent，负责模型调用、工具调用、会话、技能、记忆、计划任务等能力。
- `hermes-webui`：基于 `EKKOLearnAI/hermes-web-ui` 的浏览器管理界面，提供聊天、会话管理、工作区文件浏览、设置、模型配置、网关管理等功能。
- `start.bat` / `scripts/start-eazyhermes.ps1`：Windows 一键启动脚本。
- `packages/python`：打包时下载 Windows embeddable Python 和 `get-pip.py`。
- `packages/node`：打包时下载 Windows portable Node.js。
- `packages/wheelhouse`：打包时下载 Windows 离线 Python wheel 依赖包。
- `.github/workflows/offline-package.yml`：GitHub Actions 离线打包流程。

当前集成版本：

- `hermes-webui`: `EKKOLearnAI/hermes-web-ui@5193dbc`
- `hermes-agent`: `cebf95854bf5ee577930a7566a1dc07968821d72`

## 已包含能力

- 中文界面：WebUI 已包含简体中文 `zh` 和繁体中文 `zh-Hant`。
- 离线启动：运行包内置 Python、Node.js、Python wheels、WebUI build 输出和 WebUI 生产 npm 依赖，内网 Windows 解压即用。
...
- 轻量内网包：只打包新 WebUI 与 Hermes Agent 核心运行所需依赖，保留命令行、计划任务、Windows PTY 和 Web 终端所需的 Node 运行支持。

## 不包含的非必须依赖

为了缩短 GitHub Actions 构建时间，并降低内网分发包体积，当前离线包不会打包以下可选能力：

- Docker 镜像和 Docker 运行环境。
- 浏览器自动化工具链。
- WhatsApp、Telegram、Discord、Slack、DingTalk、Feishu、Google 等平台机器人集成。
- 重型语音/STT、premium TTS、训练、benchmark、开发测试和实验性 git-sourced extras。
- Agent dashboard / TUI 等不影响 WebUI 一键启动的附加界面。

如果以后确实需要其中某类能力，建议单独做专用构建，而不是放进默认内网离线包。
...
EazyHermes/
  ...
  packages/
    python/                     # 打包时生成：Windows embeddable Python 与 get-pip.py
    node/                       # 打包时生成：Windows portable Node.js
    wheelhouse/                 # 打包时生成：Windows 离线 Python wheel 包
...
- 下载 `requirements-windows.txt` 中声明的 Windows wheel 依赖，覆盖 WebUI 与 Hermes Agent 核心运行依赖。
...
3. 从 `packages/wheelhouse/` 离线安装 Hermes Agent 所需 Python 依赖。
4. 创建本地 `data/`、`data/.hermes/`、`data/webui/` 和 `workspace/`。
5. 设置 `HERMES_HOME`、WebUI 数据目录、Hermes CLI 路径和内置 Node/Python 路径。
6. 启动新 WebUI 并打开 `http://127.0.0.1:8787`。
